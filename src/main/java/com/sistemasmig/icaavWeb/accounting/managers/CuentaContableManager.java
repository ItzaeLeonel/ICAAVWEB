...
public class CuentaContableManager {
	...
	public PagedResponse<CuentaContable> getCuentaContable(CuentaContable filter, Paging paging) {
		...
		if (filter.getFechaModificacion() != null && filter.getFechaModificacion2() != null) {
			predicates.add(cb.between(root.get("fechaModificacion"), filter.getFechaModificacion(),
					filter.getFechaModificacion2()));
	...
	public CuentaContable createCuentaContable(CuentaContable cuentaContable)
			throws BusinessLogicException, ExistentEntityException, EntityNotExistentException {
	...
	private void validateCuentaContable(CuentaContable cuentaContable)
			throws BusinessLogicException, EntityNotExistentException {
		...
		if (cuentaContable.getGrupoEmpresa() != null) {
			if (cuentaContable.getGrupoEmpresa().getId() == null) {
				throw new BusinessLogicException(
						"El campo id de GrupoEmpresa es requerido para el objeto CuentaContable");
			}
		...
		if (cuentaContable.getSatCuentaContable() != null) {
			if (cuentaContable.getSatCuentaContable().getId() == null) {
				throw new BusinessLogicException(
						"El campo id de SatCuentaContable es requerido para el objeto CuentaContable");
			}
	...
	private void validateUnique(CuentaContable cuentaContable) throws ExistentEntityException {
		List<CuentaContable> cuentaContablees = cuentaContableRepository
				.findByNumeroCuentaAndBorrado(cuentaContable.getNumeroCuenta(), Boolean.FALSE);
	...
	public CuentaContable updateCuentaContable(Integer cuentaContableId, CuentaContable cuentaContable)
	...
	public List<ListCuentasContables> getCuentasContablesSP(Integer pr_id_grupo_empresa)
	...
	public InsertCuentaC createByCuentaContableSP(Integer pr_id_empresa, Integer pr_id_cuenta_sat, Integer pr_anio,
			Integer pr_mes, String pr_numero_cuenta, String pr_tipo_cuenta, String pr_nombre, Double pr_saldo_inicial,
			Double pr_saldo_final, Double pr_cargos, Double pr_abonos, Double pr_cargos_acumulado,
			Double pr_abonos_acumulados, Integer pr_cuenta_x_pagar, CuentaContableEstatusEnum pr_estatus,
			String pr_numero_complementaria, Integer pr_cuenta_complementaria) throws EntityNotExistentException {
		Map<String, ?> resultado = cuentaContableInsertRepository.Insertprocedurecuentascont(pr_id_empresa,
				pr_id_cuenta_sat, pr_anio, pr_mes, pr_numero_cuenta, pr_tipo_cuenta, pr_nombre, pr_saldo_inicial,
				pr_saldo_final, pr_cargos, pr_abonos, pr_cargos_acumulado, pr_abonos_acumulados, pr_cuenta_x_pagar,
				pr_estatus, pr_numero_complementaria, pr_cuenta_complementaria);
		...
		if (resultado.get("pr_message") != null && resultado.get("pr_message").equals("SUCCESS")) {
			...
			InsertCuentaC insertCuentaC = cuentasContablesInsertRepository
					.getById((Integer) resultado.get("pr_inserted_id"));
	...
	public UpdateCCSP updateByCuentaContableSP(Integer pr_id_empresa, Integer pr_id_cuenta_sat, Integer pr_anio,
			Integer pr_mes, String pr_numero_cuenta, String pr_tipo_cuenta, String pr_nombre, Double pr_saldo_inicial,
			Double pr_saldo_final, Double pr_cargos, Double pr_abonos, Double pr_cargos_acumulado,
			Double pr_abonos_acumulados, Integer pr_cuenta_x_pagar, String pr_estatus, String pr_numero_complementaria)
			throws EntityNotExistentException {

		Map<String, ?> resultado = cuentaContableUpdateRepository.Updateprocedurecuentascont(pr_id_empresa,
				pr_id_cuenta_sat, pr_id_cuenta_sat, pr_anio, pr_mes, pr_numero_cuenta, pr_tipo_cuenta, pr_nombre,
				pr_saldo_inicial, pr_saldo_final, pr_cargos, pr_abonos, pr_cargos_acumulado, pr_abonos_acumulados,
				pr_cuenta_x_pagar, CuentaContableEstatusEnum.ACTIVO, pr_numero_complementaria);
