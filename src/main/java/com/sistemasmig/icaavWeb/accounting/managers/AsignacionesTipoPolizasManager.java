...
import com.sistemasmig.icaavWeb.accounting.repositories.AsignacionesTipoPolizasRepository;
 ...
 */
@Component
public class AsignacionesTipoPolizasManager {

	@Autowired
	private AsignacionesTipoPolizasRepository asignacionesTipoPolizasRepository;

	@Autowired
	private EntityManager entityManager;

	public AsignacionTipoPolizas getById(Integer id) throws EntityNotExistentException {
		return asignacionesTipoPolizasRepository.findById(id).orElse(null);
	}
	...
	public PagedResponse<AsignacionTipoPolizas> getAsignacionTipoPolizas(AsignacionTipoPolizas filter, Paging paging) {
		...
		if (filter.getTipoPoliza() != null) {
			predicates.add(cb.equal(root.get("tipoPol"), filter.getTipoPoliza()));
		}
		...
		if (filter.getUsuario() != null) {
			if (filter.getUsuario().getAccesoHorario() != null) {
				predicates.add(
						cb.equal(root.get("usuario").get("accesoHorario"), filter.getUsuario().getAccesoHorario()));
			}
			...
			if (filter.getUsuario().getCorreo() != null) {
				predicates.add(cb.like(cb.lower(root.get("usuario").get("correo")),
						"%" + filter.getUsuario().getCorreo().toLowerCase() + "%"));
			}
			if (filter.getUsuario().getHoraAccesoFin() != null) {
				predicates.add(
						cb.equal(root.get("usuario").get("horaAccesoFin"), filter.getUsuario().getHoraAccesoFin()));
			}
			if (filter.getUsuario().getHoraAccesoIni() != null) {
				predicates.add(
						cb.equal(root.get("usuario").get("horaAccesoIni"), filter.getUsuario().getHoraAccesoIni()));
			}

			if (filter.getUsuario().getFechaModificacion() != null
					&& filter.getUsuario().getFechaModificacion2() != null) {
				predicates.add(cb.between(root.get("usuario").get("fechaModificacion"),
						filter.getUsuario().getFechaModificacion(), filter.getUsuario().getFechaModificacion2()));
			...
			if (filter.getUsuario().getIdUsuarioMod() != null) {
				predicates
						.add(cb.equal(root.get("usuario").get("idUsuarioMod"), filter.getUsuario().getIdUsuarioMod()));
			}
			if (filter.getUsuario().getInicioSesion() != null) {
				predicates
						.add(cb.equal(root.get("usuario").get("inicioSesion"), filter.getUsuario().getInicioSesion()));
			}
			if (filter.getUsuario().getIntentosIngreso() != null) {
				predicates.add(
						cb.equal(root.get("usuario").get("intentosIngreso"), filter.getUsuario().getIntentosIngreso()));
			}
			if (filter.getUsuario().getMaternoUsuario() != null) {
				predicates.add(cb.like(cb.lower(root.get("usuario").get("maternoUsuario")),
						"%" + filter.getUsuario().getMaternoUsuario().toLowerCase() + "%"));
			}
			if (filter.getUsuario().getNombreUsuario() != null) {
				predicates.add(cb.like(cb.lower(root.get("usuario").get("nombreUsuario")),
						"%" + filter.getUsuario().getNombreUsuario().toLowerCase() + "%"));
			}
			if (filter.getUsuario().getPasswordUsuario() != null) {
				predicates.add(cb.like(cb.lower(root.get("usuario").get("passwordUsuario")),
						"%" + filter.getUsuario().getPasswordUsuario().toLowerCase() + "%"));
			}
			if (filter.getUsuario().getPaternoUsuario() != null) {
				predicates.add(cb.like(cb.lower(root.get("usuario").get("paternoUsuario")),
						"%" + filter.getUsuario().getPaternoUsuario().toLowerCase() + "%"));
			}
			if (filter.getUsuario().getPrimerIngreso() != null) {
				predicates.add(
						cb.equal(root.get("usuario").get("primerIngreso"), filter.getUsuario().getPrimerIngreso()));
			}
			if (filter.getUsuario().getRegistraUsuario() != null) {
				predicates.add(cb.like(cb.lower(root.get("usuario").get("registraUsuario")),
						"%" + filter.getUsuario().getRegistraUsuario().toLowerCase() + "%"));
			}
			if (filter.getUsuario().getUsuario() != null) {
				predicates.add(cb.like(cb.lower(root.get("usuario").get("usuario")),
						"%" + filter.getUsuario().getUsuario().toLowerCase() + "%"));
			}
			...
			if (filter.getUsuario().getVisualizarDashboard() != null) {
				predicates.add(cb.equal(root.get("usuario").get("visualizarDashboard"),
						filter.getUsuario().getVisualizarDashboard()));
			}
		}

		if (filter.getTipoPoliza() != null) {
			if (filter.getTipoPoliza().getClave() != null) {
				predicates.add(cb.equal(root.get("tipoPol").get("clave"), filter.getTipoPoliza().getClave()));
			}
			if (filter.getTipoPoliza().getDesc_tipo_pol() != null) {
				predicates.add(
						cb.equal(root.get("tipoPol").get("descTipoPol"), filter.getTipoPoliza().getDesc_tipo_pol()));
			}
			if (filter.getTipoPoliza().getTipoPolizaEstatusEnum() != null) {
				predicates.add(cb.equal(root.get("tipoPol").get("tipoPolizaEstatusEnum"),
						filter.getTipoPoliza().getTipoPolizaEstatusEnum()));
			}
			if (filter.getTipoPoliza().getId() != null) {
				predicates.add(cb.equal(root.get("tipoPol").get("id"), filter.getTipoPoliza().getId()));
			}
			if (filter.getTipoPoliza().getFechaModificacion() != null
					&& filter.getTipoPoliza().getFechaModificacion2() != null) {
				predicates.add(cb.between(root.get("tipoPol").get("fechaModificacion"),
						filter.getTipoPoliza().getFechaModificacion(), filter.getTipoPoliza().getFechaModificacion2()));
		...
		return new PagedResponse<AsignacionTipoPolizas>((int) page.getTotalElements(), page.getTotalPages(),
	...
	public AsignacionTipoPolizas save(AsignacionTipoPolizas entity) {
		return asignacionesTipoPolizasRepository.save(entity);
	}

	@Transactional
	public AsignacionTipoPolizas update(AsignacionTipoPolizas entity) {
		return asignacionesTipoPolizasRepository.save(entity);
	}

	@Transactional
	public void delete(AsignacionTipoPolizas entity) {
		asignacionesTipoPolizasRepository.save(entity);
	}

	public List<AsignacionTipoPolizas> findAll() {
		return asignacionesTipoPolizasRepository.findAll();
	}
